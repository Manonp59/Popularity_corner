...
class BoxOfficeSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        yield scrapy.Request(url=box_office_url, callback=self.parse_box_office)

    def parse_box_office(self, response):
        items = BoxOfficeItem()
        date = self.last_thursday_date()

        items["title"] = response.css("div.gd-col-left section:nth-of-type(1) tr:nth-of-type(1) td.first-col::text").extract()
        items["week"] = f"{date}"
