...
class BoxOfficeSpider(CrawlSpider):
    ...
    rules = (
        ...
        Rule(LinkExtractor(restrict_xpaths=f"//nav[@class='pagination cf']//a[contains(text(), 'Suivant')]/@href"), callback='parse_next_page', follow=True),
    )
    ...
    def parse_next_page(self, response):
        ...
        next_page_url = response.xpath("//nav[@class='pagination cf']//a[contains(text(), 'Suivant')]/@href").get()
        if next_page_url is not None:
            yield scrapy.Request(url=next_page_url, headers={'User-Agent': self.user_agent_list[page % len(self.user_agent_list)]})