...
class BoxOfficeSpider(CrawlSpider):
    ...
    def parse_box_office(self, response):
        ...
        items["title"] = response.css('div.titlebar-page div.titlebar-title-lg a.title-bar-link::text').extract()
        items["week"] = f"{date}"
        items["entranc"] = response.css('.gd-col-left section:nth-of-type(1) tr:nth-of-type(1) td.second-col::text').extract_first()
        yield items
