...
class BoxOfficeSpider(CrawlSpider):
    ...
    def parse_box_office(self, response):
        ...
        items["title"] = response.xpath("document.querySelector("#content-layout > div.titlebar.titlebar-page > div.titlebar-title.titlebar-title-lg > a").extract()[0]").extract_first()
        items["week"] = f"{date}"
